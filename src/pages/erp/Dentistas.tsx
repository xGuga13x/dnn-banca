...
export default function Dentistas() {
  ...
  const openEdit = (d: Dentista) => {
    setEditing(d)
    reset({ nome: d.nome, cpf: d.cpf, email: d.email, telefone: d.telefone,
      dataNascimento: d.dataNascimento, cro: d.cro, especialidade: d.especialidade })
    setModalOpen(true)
  }
  ...
  const onSubmit = (data: FormData) => {
    if (editing) {
      setDentistas((prev) => prev.map((d) => d.id === editing.id ? { ...d, ...data } : d))
    } else {
      setDentistas((prev) => [...prev, { ...data, id: Date.now(), idDentista: Date.now(), ativo: true }])
    }
    setModalOpen(false)
  }
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="font-display font-extrabold text-tdb-teal text-3xl">Dentistas</h1>
      ...
      <input type="text" placeholder="Buscar por nome ou CRO..."
        value={search} onChange={(e) => setSearch(e.target.value)} className={inputClass + ' max-w-sm'} />

      {filtered.length === 0
        ? <EmptyState icon="👨‍⚕️" title="Nenhum dentista encontrado" />
        : (
                    ...
                    <tr key={d.id} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-6 py-4">
                        <Badge label={d.ativo ? 'Ativo' : 'Inativo'}
                          className={d.ativo ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-500'} />
                      </td>
                      <td className="px-6 py-4">
                        <div className="flex gap-2">
                          <button onClick={() => openEdit(d)} className="text-tdb-teal hover:underline font-medium text-xs">Editar</button>
                          {d.ativo && <button onClick={() => handleDelete(d.id)} className="text-red-400 hover:underline font-medium text-xs">Desativar</button>}
                        </div>
                      </td>
                    </tr>
                  ))}
