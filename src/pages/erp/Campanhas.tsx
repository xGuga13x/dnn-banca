...
const mock: Campanha[] = [
  { id: 1, idCampanha: 1, nome: 'Sorriso Solidário Maio', descricao: 'Campanha maio/2025',    dtInicio: '2025-05-01', dtFim: '2025-05-31', metaValor: 20000, totalArrecadado: 12500, ativo: true },
  { id: 2, idCampanha: 2, nome: 'Dia das Mães TDB',       descricao: 'Campanha Dia das Mães', dtInicio: '2025-05-10', dtFim: '2025-05-12', metaValor: 10000, totalArrecadado: 8750,  ativo: true },
  { id: 3, idCampanha: 3, nome: 'Campanha Abril',          descricao: 'Campanha abril/2025',   dtInicio: '2025-04-01', dtFim: '2025-04-30', metaValor: 15000, totalArrecadado: 16200, ativo: false },
]
type FormData = { nome: string; descricao: string; dtInicio: string; dtFim: string; metaValor: number }
...
export default function Campanhas() {
  ...
  const openEdit       = (c: Campanha) => { setEditing(c); reset({ nome: c.nome, descricao: c.descricao ?? '', dtInicio: c.dtInicio, dtFim: c.dtFim, metaValor: c.metaValor }); setModalOpen(true) }
  const handleEncerrar = (id: number) => { if (!confirm('Encerrar campanha?')) return; setLista(prev => prev.map(c => c.id === id ? { ...c, ativo: false } : c)) }
  const onSubmit       = (data: FormData) => {
    if (editing) {
      setLista(prev => prev.map(c => c.id === editing.id ? { ...c, ...data } : c))
    } else {
      const newId = Date.now()
      setLista(prev => [...prev, { ...data, id: newId, idCampanha: newId, totalArrecadado: 0, ativo: true }])
    }
    setModalOpen(false)
  }
      ...
      {filtered.length === 0 ? <EmptyState icon="📣" title="Nenhuma campanha encontrada" /> : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {filtered.map(c => {
            ...
            const arrecadado = c.totalArrecadado ?? 0
            ...
            return (
              <div key={c.id} className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
                ...
                <p className="text-gray-400 text-xs font-body mb-4">{c.dtInicio} até {c.dtFim}</p>
                <div className="mb-1 flex justify-between text-xs font-body text-gray-500">
                  <span>{formatCurrency(arrecadado)}</span>
                ...
                <div className="flex gap-2">
                  ...
                  {c.ativo && <Button size="sm" variant="danger" onClick={() => handleEncerrar(c.id)}>Encerrar</Button>}
                </div>
              </div>
            )
          })}
